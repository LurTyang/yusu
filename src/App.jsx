...
import { BookOpen, Upload, Home, User, Heart, Download, Tag, MessageSquare, ThumbsUp, ChevronLeft, ChevronRight, Search, Clock, FileText, Layers, Filter, LogOut, LogIn, AlignLeft, Edit } from 'lucide-react';
  ...
  return (
    <div className="max-w-md mx-auto p-6 mt-20 animate-in fade-in">
      <div className={`p-8 rounded-2xl shadow-sm ${t.cardStatic}`}>
        ...
        <p className={`text-sm mb-6 ${t.textMuted}`}>{authMode === 'login' ? '欢迎回到青鸟学会图书馆。' : '只有白名单内的用户才可注册。请联系管理员登记。'}</p>
        ...
        <div className={`mt-6 pt-4 border-t text-center text-sm ${t.border}`}>
          <span className={t.textMuted}>{authMode === 'login' ? '还没有账号？' : '已有账号？'}</span>
          <button type="button" onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className={`ml-2 font-bold ${t.accent} hover:underline`}>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6 animate-in fade-in">
      <div className={`flex flex-col md:flex-row justify-between items-center gap-4 p-6 rounded-2xl ${t.cardStatic}`}>
        <div>
          <h1 className={`text-3xl font-bold flex items-center gap-3 ${t.heading}`}><BookOpen className={t.accent} /> 真羽叙事</h1>
        ...
        <div className="w-full md:w-auto flex-1 max-w-lg flex gap-2">
          <div className="relative w-full">
            ...
            <input type="text" placeholder="搜索书名、简介、标签……" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-10 pr-4 py-3 rounded-xl transition-all ${t.input}`} />
          </div>
          <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className="px-4" title="高级过滤"><Filter size={20} /></Button>
        </div>
      </div>
      ...
      {showFilters && (
        <div className={`p-5 rounded-xl flex flex-wrap gap-4 items-end animate-in slide-in-from-top-2 ${t.cardStatic} text-sm`}>
          ...
          <Button variant="ghost" onClick={() => { setFilterIncludeTag(''); setFilterExcludeTag(''); setMinWords(''); setMaxWords(''); setSortBy('time'); }} className="ml-auto">重置</Button>
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginatedNovels.length === 0 ? (
          <div className={`col-span-full py-20 text-center ${t.textMuted}`}><BookOpen size={48} className="mx-auto mb-4 opacity-20" /><p>没有找到符合条件的内容</p></div>
        ) : (
            ...
            return (
              <div key={novel.id} className={`${t.card} p-5 flex flex-col h-full group`}>
                <div className="flex justify-between items-start mb-3">
                  <span className={`${t.iconBg} text-xs px-2.5 py-1 rounded-full font-medium`}>{novel.genre}</span>
                  <button onClick={() => toggleFavorite(novel.id)} className={`${t.textLight} hover:text-red-500 transition-colors flex items-center gap-1`}>
                ...
                <div className="flex flex-wrap gap-2 mb-4 h-6 overflow-hidden">
                  {topTags.map((tag, i) => (
                    <span key={i} className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${t.secondaryBtn.split(' ')[0]} ${t.textMuted}`}><Tag size={10} /> {tag.text}</span>
                  ))}
                ...
                <div className={`mt-auto pt-4 border-t flex items-center justify-between text-xs ${t.borderLight} ${t.textMuted}`}>
                  <div className="flex items-center gap-1"><User size={14} /> {novel.author || novel.uploaderName}</div>
                  <div className="flex items-center gap-1"><Clock size={14} /> {new Date(novel.uploadDate).toLocaleDateString()}</div>
                </div>
                
                <Button variant="outline" className="w-full mt-4" onClick={() => navigate(`/novel/${novel.id}`)}>开始阅读</Button>
              </div>
            );
          })
        )}
      ...
      {totalPages > 1 && (
        <div className={`flex justify-center items-center gap-4 py-6 ${t.textMuted}`}>
          <Button variant="ghost" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}><ChevronLeft size={20} /></Button>
          <span className="font-medium text-sm">第 {currentPage} / {totalPages} 页</span>
          <Button variant="ghost" disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)}><ChevronRight size={20} /></Button>
        </div>
      )}
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex justify-between h-16">
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
              <div className={`p-2 rounded-lg ${t.navLogo}`}><BookOpen size={20} /></div>
            ...
            <div className="flex items-center gap-1 md:gap-4">
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <div className={`w-px h-6 mx-1 md:mx-2 ${t.navDivider}`}></div>
                  <button onClick={() => navigate('/profile')} className={`p-2 md:px-4 md:py-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${isActive('/profile') ? t.navItemActive : t.navItem}`}>
                    <User size={18} /> <span className="hidden md:inline">{userProfile.name}</span>
                  </button>
                  <button onClick={handleLogout} title="退出登录" className={`p-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 hover:text-red-500`}>
                    <LogOut size={18} />
                  </button>
                </>
              ) : (
                <>
                  <div className={`w-px h-6 mx-1 md:mx-2 ${t.navDivider}`}></div>
                  <button onClick={() => navigate('/auth')} className={`p-2 md:px-4 md:py-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${isActive('/auth') ? t.navItemActive : t.navItem}`}>
